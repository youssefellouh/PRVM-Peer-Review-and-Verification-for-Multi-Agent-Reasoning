...
if __name__ == "__main__":
    ...
    if not os.path.isabs(args.dataset_dir):
        args.dataset_dir = os.path.join(repo_root, args.dataset_dir)
    if not os.path.isabs(args.output_dir):
        args.output_dir = os.path.join(repo_root, args.output_dir)

    time_flag = time.strftime('%m%d')
    args.output_file = os.path.join(
        args.output_dir,
        args.task,
        f'{args.task}_PRVM_Without_Confidence_{args.max_example_num}_{time_flag}.json'
    )
    output_task_dir = os.path.dirname(args.output_file)
    os.makedirs(output_task_dir, exist_ok=True)

    log_param(args)
    logging.info(f"Results will be saved to: {args.output_file}")
